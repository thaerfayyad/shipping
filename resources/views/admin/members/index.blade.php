            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <h2 class="card-title">{{ __('member/config.title') }} <a href="{{route('members.create')}}" class="btn btn-info waves-effect waves-light">{{__('config.create')}} <i class="fas fa-plus-square"></i></a></h2>
                        <div class="table-responsive">
                            ...
                            <table id="file_export" class="table table-striped table-bordered display">
                                ...
                                <tr>
                                    ...
                                    <th>{{__('member/config.id')}}</th>
                                    <th>{{__('member/config.actions')}}</th>
                                    <th>{{__('member/config.FullName')}}</th>
                                    <th>{{__('member/config.Address')}}</th>
{{--                                    <th>{{__('admin/member.config.name_en')}}</th>--}}
                                ...
                                @foreach ($members as $key => $val)
                                    <tr>
                                        ...
                                        <td>{{$key+1}}</td>
                                        <td>
                                            <a href="{{url(LaravelLocalization::setLocale().'/admin/members/'.$val->MemberID.'/edit')}}" class="btn btn-info waves-effect waves-light"><i class="fas fa-edit"></i></a>
                                             <form action="{{ route('members.destroy',$val->MemberID) }}" method="post" class="form-check-inline" >
                                                    @csrf
                                                    {{ method_field('DELETE') }}
...
{{--                                <tfoot>--}}
{{--                                <tr>--}}
{{--                                    <th>{{__('admin/config.packages.id')}}</th>--}}
{{--                                    <th>{{__('admin/config.packages.name')}}</th>--}}
{{--                                    <th>{{__('admin/config.packages.length')}}</th>--}}
{{--                                    <th>{{__('admin/config.packages.width')}}</th>--}}
{{--                                    <th>{{__('admin/config.packages.height')}}</th>--}}
{{--                                </tr>--}}
{{--                                </tfoot>--}}
                            </table>
                        </div>
